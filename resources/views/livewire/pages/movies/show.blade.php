...
<div class="movie-info border-b border-gray-800" 
    style="background: linear-gradient(rgba(26, 32, 44,.9), rgba(26, 32, 44,.9)), url( {{{ $movie['backdrop_path'] }}}) no-repeat center/cover"
>
    <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
        <div class="flex-none">
            ...
            <ul class="flex items-center mt-4">
                @if ($movie['homepage'])
                    <li>
                        <a href="{{ $movie['homepage'] }}" title="Website">
                ...
                @endif
                @if ($movie['social']['facebook'])
                    <li class="ml-6">
                        <a href="{{ $movie['social']['facebook'] }}" title="Facebook">
                ...
                @endif
                @if ($movie['social']['instagram'])
                    <li class="ml-6">
                        <a href="{{ $movie['social']['instagram'] }}" title="Instagram">
                ...
                @endif
                @if ($movie['social']['twitter'])
                    <li class="ml-6">
                        <a href="{{ $movie['social']['twitter'] }}" title="Twitter">
        ...
        <div class="md:ml-24">
            ...
            <div class="mt-12">
                ...
                <div class="flex flex-wrap text-gray-400">
                    ...
                    <div class="mr-8 mt-4">
                        ...
                        <div class="text-sm text-gray-400">{{ $movie['original_title'] }}</div>
                    </div>
                    @endif
                    @if($movie['production_countries'])
                    <div class="mr-8 mt-4">
                        <h4 class="text-white ">Production countries</h4>
                        <div class="text-sm text-gray-400">{{ $movie['production_countries'] }}</div>
                    </div>
                    @endif
                    @if($movie['spoken_languages'])
                    <div class="mr-8 mt-4">
                        <h4 class="text-white ">Languages</h4>
                        <div class="text-sm text-gray-400">{{ $movie['spoken_languages'] }}</div>
                    </div>
                    @endif
                </div>
            </div>
            <!-- Keywords -->
            @if(count($movie['keywords']) > 0)
            <div class="mt-12">
                ...
                <ul>
                    <div class="flex flex-wrap mt-4">
                        ...
                        <li><a class="inline-block border border-gray-300 text-gray-400 rounded-md px-4 py-2 mr-2 mt-2 transition duration-500 ease select-none hover:bg-gray-300 hover:text-gray-900 focus:outline-none focus:shadow-outline"
                             href="{{ route('movies.discover.index', ['keywordId'=>$keyword['id'],'keywordName'=>'keyword '.$keyword['name']])}}">{{$keyword['name']}}</a>
                        </li>
                        @endforeach 
                    </div>
                </ul>
            ...
            @endif
            <!-- Movie trailer btn & modal, with alpinejs state x-data-->
            <div x-data="{ isOpen: false }">
                @if (count($movie['videos']['results']) > 0)
                    ...
                    <template x-if="isOpen">    
                        <div x-show.transition.opacity="isOpen"  style="background-color: rgba(0, 0, 0, .5);" class="z-10 fixed top-0 left-0 w-full h-full flex items-center shadow-lg overflow-y-auto">
                            <div class="container mx-auto lg:px-32 rounded-lg overflow-y-auto">
                                <div class="bg-gray-900 rounded">
                                    ...
                                    <div class="modal-body px-8 py-8">
                                        ...
                                        <!-- For stopping video on frame after clicking close button , use this :  $('.youtube-video')[0].contentWindow.postMessage('{"event":"command","func":"' + 'stopVideo' + '","args":""}', '*'); -->
...
<div class="tv-cast border-b border-gray-800">
    <div class="container mx-auto px-4 py-16">
        ...
        <div x-data="{swiper: null}"
            x-init="swiper = new Swiper($refs.container, swiperSetup({{count($movie['cast'])}}))"
...
<div class="tv-images border-b border-gray-800">
    <div class="container mx-auto px-4 py-16" >
        ...
        <div x-data="{swiper: null, isOpen: false, image: '',}" 
            x-init="swiper = new Swiper($refs.container, swiperSetupForImages({{ count($movie['images']) }}))"
...
@endif <!-- end movie-images -->

<!-- similar-movies -->
@if(count($similarMovies) > 1)
<div class="container mx-auto px-4 py-16">
    <h2 class="text-3xl font-semibold">Similar Movies</h2>
    <div x-data="{swiper: null}"
        x-init="swiper = new Swiper($refs.container, swiperSetup({{count($similarMovies)}}))"
        class="relative mx-auto flex flex-row">
