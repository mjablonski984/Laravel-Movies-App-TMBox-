<div>
        ...
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            <div class="flex-none">
                ...
                <ul class="flex items-center mt-4">
                    ...
                    @if ($actor['social']['facebook'])
                        <li>
                            <a href="{{ $actor['social']['facebook'] }}" title="Facebook">
                    ...
                    @endif
                    @if ($actor['social']['instagram'])
                        <li class="ml-6">
                            <a href="{{ $actor['social']['instagram'] }}" title="Instagram">
                    ...
                    @endif
                    @if ($actor['social']['twitter'])
                        <li class="ml-6">
                            <a href="{{ $actor['social']['twitter'] }}" title="Twitter">
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    @if($actor['birthday'] || $actor['place_of_birth'] )
                    ...
                    @endif
                    <span class="ml-2">{{ $actor['birthday'] }}
                        @if($actor['deathday'] != null) - {{ $actor['deathday'] }} @endif
                        @if($actor['birthday'] != null) ({{ $actor['age'] }} years old) @endif
                        @if($actor['place_of_birth'] != null) in {{ $actor['place_of_birth'] }} @endif
                    </span>
                </div>
                ...
                <div class="grid grid-cols-3 xl:grid-cols-6 gap-8">
    ...
    <div class="tv-images border-b border-gray-800">
        <div class="container mx-auto px-4 py-16" >
            ...
            <div x-data="{swiper: null, isOpen: false, image: '',}" 
                x-init="swiper = new Swiper($refs.container, swiperSetup({{ count($actor['images']) }}))"
                    ...
                    <!-- Slides -->
                        @foreach ($actor['images'] as $image)
                            <div class="swiper-slide">
                                ...
                                <a href="" class="inline-block w-full sm:w-auto max-w-full sm:max-w-64"
                                    @click.prevent="
                                        isOpen = true
                                        image='{{ $image['file_path'] }}'
                                    "
                                >
                                    <img src="{{ $image['file_path'] }}" alt="image" class="inline-block w-full hover:opacity-75 transition ease-in-out duration-150">
                                </a>
                                </div>
                            </div>               
                        @endforeach
                ...
                <div x-show="isOpen" style="background-color: rgba(0, 0, 0, .5);" class="z-10 fixed top-0 left-0 w-full h-full flex items-center shadow-lg overflow-y-auto">
                    <div class="container mx-auto max-w-lg rounded-lg overflow-y-auto">
                        <div class="bg-gray-900 rounded">
                            ...
                            <div class="modal-body px-8 py-8">
                                <img :src="image" alt="poster" class="w-full max-w-md">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @endif <!-- end actor-images -->

    @if(count($credits) > 0)
    <div class="credits"  x-data="{ isOpen: false }">
        <div class="container mx-auto px-4 py-16">
            <h2 class="text-3xl font-semibold">Credits
                <span @click="isOpen=!isOpen" class="ml-6 font-semibold inline-block align-baseline cursor-pointer hover:opacity-75 transition ease-in-out duration-150">           
                    <svg x-show="isOpen" id="icon-circle-up" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                        <path d="M0 12c0 6.627 5.373 12 12 12s12-5.373 12-12-5.373-12-12-12-12 5.373-12 12zM21.75 12c0 5.385-4.365 9.75-9.75 9.75s-9.75-4.365-9.75-9.75 4.365-9.75 9.75-9.75 9.75 4.365 9.75 9.75z"></path><path d="M16.564 15.686l2.121-2.121-6.686-6.686-6.686 6.686 2.121 2.121 4.564-4.564z"></path>
                    </svg>
                    <svg x-show="!isOpen" id="icon-circle-down" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                        <path d="M24 12c0-6.627-5.373-12-12-12s-12 5.373-12 12 5.373 12 12 12 12-5.373 12-12zM2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75-9.75-4.365-9.75-9.75z"></path><path d="M7.436 8.314l-2.121 2.121 6.686 6.686 6.686-6.686-2.121-2.121-4.564 4.564z"></path>
                    </svg>                          
                </span>
            </h2>
            <ul class="list-disc leading-loose pl-5 mt-8 transform origin-top"
                x-show="isOpen"
            ...
            >    
                @foreach ($credits as $credit)
                    ...
                    <a href="{{ route('movies.show', $credit['id']) }}"><strong class="hover:text-gray-300">{{ $credit['title'] }}</strong></a>
                    @endif
                    @if($credit['media_type'] == 'tv')    
                    <a href="{{ route('tv.show', $credit['id']) }}"><strong class="hover:text-gray-300">{{ $credit['title'] }}</strong></a>
                    @endif
                    @if($credit['character'])
                    as {{ $credit['character'] }}
                    @endif
